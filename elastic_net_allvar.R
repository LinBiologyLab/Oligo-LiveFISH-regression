...
ggsave(sprintf('result/cor_pred_all_%s.pdf',selY),width = 3, height = 2.3, units = "in")
write.csv(x = data.frame(y_pred= c(ytrain_pred,ytest_pred), 
                         y_true=c(ytrain,ytest), 
                         type=c(rep('train' , length(ytrain_pred)),rep('test', length(ytest_pred)))),
          file = sprintf('result/data_all_%s.csv',selY)
            )
...
ggsave(sprintf('result/feature_importance_all_%s.pdf',selY),width = 2, height = 1.8, units = "in")
